...
Thrust commands travel through three stages before becoming water flow, as sketched in Figure~\ref{fig:actuation}: (i) the velocity control computes a desired thrust in Newtons per side; (ii) the \texttt{motor\_thrust2RPM} logic converts that thrust command into a bounded, rate-limited ESC duty cycle; (iii) the VESC driver forwards the duty cycle to the ESC, which runs closed-loop FOC commutation on the thruster motors.


\begin{figure}[H]
\centering
% \textwidth forces it to match your page margins exactly
\resizebox{\textwidth}{!}{
...
The duty cycle is then rate-limited so the propeller never receives a step input.
A ramp mode was used in which the allowed rate of change grows quadratically with the remaining error, giving a soft start and a snappy finish:
...
where $\dot d_{\max}=$ \texttt{max\_duty\_cycle\_rate} $=0.9\,\text{s}^{-1}$. A zero-crossing (sign reversal of the command) triggers an instantaneous stop-before-reverse modeling the fact that water drag decelerates the hull quickly compared to thruster spin-up; same-direction deceleration is smoothed. The result is clipped to $d\in[d_{\min},d_{\max}]=[-0.9,\,0.9]$ before being published as the duty-cycle command.
...
\texttt{ramp\_mode} & quadratic & Quadratic acceleration to avoid instant torque \\
...
\end{equation}
with the reference values used to generate the calibration table: $\text{erpm}_{\max}=23300$ (matching the VESC \texttt{speed\_max}$\,\approx 23250$ configured above) and $T_{\max}=200\,\text{N}$. This curve is what the FOC controller's closed-loop speed regulation and the thrust$\to$duty-cycle node above are calibrated against.
